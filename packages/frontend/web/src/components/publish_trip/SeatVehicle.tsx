...
export default function SeatVehicle({ seatCar }: SeatVehicleProps) {
  ...
  setValue('reservation_seat', seatAvailable);
  setValue('seat_available', seatAvailable.length);
...

{
  ...
  name={`seatCar${seat}`}
  control={control}
  defaultValue={seatCar.includes(seat)}
  render={({ field }) => (
    <>
      <input
        type='checkbox'
        ...
        value={seat}
        checked={seatVailable.includes(seat)}
        className='absolute h-full w-full opacity-0'
        disabled={seat === seatCar[0]}
      />
      <img
        src={seatVailable.includes(seat) ? '/seat.svg' : '/no-seat.svg'}
        className='h-full w-full'
      />
    </>
  )}
